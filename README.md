...
You give it a decision — often a vague one, like "improve this app" or "I want better security, I'm unsure." Instead of guessing or dumping a checklist, it stops and figures out what you're actually deciding.
...
Then it goes looking for what you missed. It sorts the situation into known and unknown quadrants and runs a quick pre-mortem — *"it's a year from now and this failed; why?"* — to drag the blind spots into the open. Depending on the kind of decision, it reaches for a few thinking lenses that fit — inversion for a risky bet, base rates for a forecast, Pareto when the work could sprawl — instead of running the same tired checklist every time.
...
```
/clear-thinking should we build our own auth or use Clerk?
/clear-thinking i want to improve this app
/clear-thinking is it worth rewriting this in Rust
```
...
- **A toolkit of lenses, not one fixed process.** [`references/thinking-styles.md`](skills/clear-thinking/references/thinking-styles.md) holds eight thinking styles — inversion, pre-mortem, second-order, first-principles, base rates, expected value, reversibility, and Pareto — each with a "when to use it" line and a runnable prompt. The skill picks the two or three that fit the problem instead of running all of them.
